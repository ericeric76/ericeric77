...
document.querySelectorAll("nav a").forEach(link => {
  link.addEventListener("click", function(e) {
    ...
    // Ambil tinggi semua elemen fixed
    const header = document.querySelector("header");
    const profile = document.querySelector(".profile-container"); // ganti sesuai class profil Eric
    const pembatas = 20; // kalau ada gap tambahan

    const totalOffset = 
        (header ? header.offsetHeight : 0) +
        (profile ? profile.offsetHeight : 0) +
        pembatas;
